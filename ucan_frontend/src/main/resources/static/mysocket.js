...
function connect() {
    var socket = new SockJS('http://127.0.0.1:8089/negotiatechat');
    ...
    stompClient.connect({"user" : document.getElementById("sender").value}, function (frame) {
        ...
        stompClient.subscribe('/user/queue/reply', function (greeting) {
            showGreeting(JSON.parse(greeting.body).ownerUser.email,JSON.parse(greeting.body).messageText,"Me");
        });
    });
}
...
function sendName() {
    ...
    stompClient.send("/negotiationmessage", {}, JSON.stringify({'messageText': $("#msgContent").val(), 'negoSession':{'id':1},'ownerUser': {'id':owneruser}}));
